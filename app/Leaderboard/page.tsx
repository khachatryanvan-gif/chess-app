...
export default function LeaderboardPage() {
  ...
  return (
    <main style={backgroundStyle} className="min-h-screen text-slate-100 flex flex-col items-center p-4 md:p-8 font-mono">
      ...
      <div className="w-full max-w-4xl bg-slate-900/80 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 shadow-2xl">
        ...
        {loading ? (
          <div className="text-center py-12 text-slate-400 animate-pulse">Loading leaderboard...</div>
        ) : users.length === 0 ? (
          <div className="text-center py-12 text-slate-400">No players found yet.</div>
        ) : (
                ...
                {users.map((user, index) => (
                  <tr key={user.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3 px-4 flex items-center gap-3">
                      ...
                      <span className="font-bold text-slate-200">{user.username || "Anonymous"}</span>
                    </td>
                    <td className="py-3 px-4 text-amber-400 font-extrabold">⭐ {user.rating ?? 1500}</td>
                    <td className="py-3 px-4 text-emerald-400 font-mono">💰 {user.balance ?? 0} USDT</td>
                  </tr>
                ))}
              </tbody>
            </table>
